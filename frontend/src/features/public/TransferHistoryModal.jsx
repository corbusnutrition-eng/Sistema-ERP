...
function recipientLabel(row) {
  ...
  if (name && username && name !== username) {
    return { primary: name, secondary: `@${username}` }
  }
  if (name) return { primary: name, secondary: null }
  if (username) return { primary: `@${username}`, secondary: null }
  return { primary: 'Sub-cliente', secondary: null }
}

export default function TransferHistoryModal({ open, onClose, token, api }) {
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      const { data } = await api.get(
        `/api/v1/portal/${encodeURIComponent(token)}/transfer-history`,
        { params: { limit: 10 } },
      )
      setItems(Array.isArray(data?.items) ? data.items : [])
    } catch (err) {
      const detail = err?.response?.data?.detail
      setError(typeof detail === 'string' ? detail : 'No se pudo cargar el historial de transferencias.')
      setItems([])
    } finally {
      setLoading(false)
    }
    ...
    <div className="fixed inset-0 z-[130] flex items-end justify-center p-0 sm:items-center sm:p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative z-10 flex max-h-[88vh] w-full max-w-md flex-col rounded-t-3xl border border-violet-400/35 bg-slate-900 shadow-2xl sm:rounded-2xl">
        <div className="flex items-center justify-between gap-3 border-b border-slate-700/50 px-5 py-4">
          <div>
            <h2 className="m-0 text-lg font-extrabold text-violet-50">Historial de transferencias</h2>
            <p className="m-1 mb-0 text-xs text-slate-400">Últimas 10 transferencias de saldo enviadas</p>
          </div>
          <button
            type="button"
            onClick={() => onClose?.()}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-5">
          {loading ? (
            ...
            <p className="m-0 rounded-xl border border-white/10 bg-black/20 px-4 py-8 text-center text-sm text-slate-400">
              Aún no has realizado transferencias de saldo.
            </p>
          ) : (
            <ul className="m-0 list-none space-y-3 p-0">
              {items.map((row) => {
                const tid = Number(row?.id)
                const { primary, secondary } = recipientLabel(row)
                return (
                  <li
                    key={`transfer-history-${tid}`}
                    className="rounded-xl border border-white/10 bg-slate-950/55 px-3.5 py-3.5"
                  >
                    <div className="flex items-center gap-3">
                      <div className="min-w-0 flex-1">
                        <p className="m-0 truncate text-[14px] font-bold text-violet-50">{primary}</p>
                        {secondary ? (
                          <p className="m-0 mt-0.5 truncate text-[11px] text-slate-400">{secondary}</p>
                        ) : null}
                        <p className="m-0 mt-1 text-[11px] text-slate-500 tabular-nums">
                          {formatShortDate(row?.date)}
                        </p>
                      </div>
                      <p className="m-0 shrink-0 text-right text-[14px] font-bold tabular-nums text-red-400">
