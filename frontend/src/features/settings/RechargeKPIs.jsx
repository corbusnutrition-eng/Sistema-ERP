...
const KPI_ITEMS = [
  { key: 'total', label: 'Total solicitudes', color: 'text-blue-600', bg: 'bg-blue-50', format: (v) => v },
...
/** Tarjetas KPI — solicitudes BaaS (mismo layout que Ventas). */
export default function RechargeKPIs({ metrics }) {
  const m = { ...DEFAULT_METRICS, ...(metrics ?? {}) }

  return (
    <div
      className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 min-h-[6.5rem]"
      aria-label="Indicadores de recargas BaaS"
    >
